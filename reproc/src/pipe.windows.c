...
DECLSPEC_IMPORT BOOLEAN WINAPI RtlGenRandom(PVOID RandomBuffer,
...
static int socketpair(int domain, int type, int protocol, SOCKET *out)
{
  ...
  int reuse = 1;
  uint32_t loopback_address = INADDR_LOOPBACK;

  ASSERT(out);

#if 0
  /* Windows XP firewall will complain loopback address other than 127.0.0.1 */
  if (reproc_windows_version_greator_or_equal(_WIN32_WINNT_VISTA, 0)) {
    uint16_t loopback_increment = (uint16_t) (rand() & 0xFFFF);
